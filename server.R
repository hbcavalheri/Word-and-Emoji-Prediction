...
shinyServer(function(session, input, output) {
    ...
    output$emoji1 <- renderUI({
            actionButton("action3", label = predemoj()[1,], width = '100px') 
    })
    
    observe(shinyjs::toggle("emoji1", condition = !is.null(predemoj()[1,1])))
    
    output$emoji2 <- renderUI({
        actionButton("action4", label = predemoj()[2,], width = '100px')
    })
    
    observe(shinyjs::toggle("emoji2", condition = !is.na(predemoj()[2,1])))
